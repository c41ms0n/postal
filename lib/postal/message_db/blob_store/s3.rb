    ...
    module BlobStore
      ...
      class S3
        ...
        def initialize(uri)
          ...
          @options["access_key_id"] ||= uri.user.to_s unless uri.user.to_s.empty?
          @options["secret_access_key"] ||= uri.password.to_s unless uri.password.to_s.empty?
        end
        ...
        #
        def store(data)
          key = File.join(*shards(SecureRandom.urlsafe_base64(32)))
