    ...
    describe "PROXY" do
      context "when the proxy header is sent correctly" do
        it "sets the IP address" do
          expect(client.handle("PROXY TCP4 1.1.1.1 2.2.2.2 1111 2222")).to eq "220 2.0.0 #{Postal::Config.postal.smtp_hostname} ESMTP Postal/#{client.trace_id}"
      ...
      context "when the proxy header is not valid" do
        it "returns an error" do
          expect(client.handle("PROXY TCP4")).to eq "502 5.5.2 Proxy Error"
