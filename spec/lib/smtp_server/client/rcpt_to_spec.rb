    ...
    describe "RCPT TO" do
      ...
      context "when MAIL FROM has not been sent" do
        ...
        it "returns an error if RCPT TO is sent before MAIL FROM" do
          expect(client.handle("RCPT TO: no-route-here@internal.com")).to eq "503 5.5.1 EHLO/HELO and MAIL FROM first please"
          expect(client.state).to eq :welcomed
        end
      end

      it "returns an error if RCPT TO is not valid" do
        expect(client.handle("RCPT TO: blah")).to eq "501 5.1.3 Invalid RCPT TO"
      end
      ...
      context "when the RCPT TO address is the system return path host" do
        it "returns an error if the server does not exist" do
          expect(client.handle("RCPT TO: nothing@#{Postal::Config.dns.return_path_domain}")).to eq "550 5.1.1 Invalid server token"
        end
        ...
        it "returns an error if the server is suspended" do
          ...
          expect(client.handle("RCPT TO: #{server.token}@#{Postal::Config.dns.return_path_domain}"))
            .to eq "535 5.7.8 Mail server has been suspended"
        end
      ...
      context "when the RCPT TO address is on a host using the return path prefix" do
        it "returns an error if the server does not exist" do
          ...
          expect(client.handle("RCPT TO: #{address}")).to eq "550 5.1.1 Invalid server token"
        end

        it "returns an error if the server is suspended" do
          server = create(:server, :suspended)
          address = "#{server.token}@#{Postal::Config.dns.custom_return_path_prefix}.example.com"
          expect(client.handle("RCPT TO: #{address}")).to eq "535 5.7.8 Mail server has been suspended"
        end
      ...
      context "when the RCPT TO address is within the route domain" do
        it "returns an error if the route token is invalid" do
          ...
          expect(client.handle("RCPT TO: #{address}")).to eq "550 5.1.1 Invalid route token"
        end
        ...
        it "returns an error if the route is set to Reject mail" do
          ...
          address = "#{route.token}@#{Postal::Config.dns.route_domain}"
          expect(client.handle("RCPT TO: #{address}")).to eq "550 5.7.1 Route does not accept incoming messages"
        end
      ...
      context "when authenticated and the RCPT TO address is provided" do
        # AUTH is only permitted outside a mail transaction, so the outer
        # MAIL FROM is deferred until after the client has authenticated.
        let(:mail_from) { nil }
        ...
        it "returns an error if the server is suspended" do
          ...
          expect(client.handle("RCPT TO: outgoing@example.com")).to eq "535 5.7.8 Mail server has been suspended"
        end
        ...
        it "adds a recipient if all OK" do
          ...
          expect(client.handle("AUTH PLAIN #{credential.to_smtp_plain}")).to match(/235 2\.7\.0 Granted for /)
          client.handle("MAIL FROM: test@example.com")
          expect(client.handle("RCPT TO: outgoing@example.com")).to eq "250 2.1.5 OK"
      ...
      context "when not authenticated and RCPT TO does not match a route" do
        it "returns an error" do
          expect(client.handle("RCPT TO: nothing@nothing.com")).to eq "530 5.7.0 Authentication required"
        end
